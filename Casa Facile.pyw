...
class GestioneSpese(tk.Tk):
    ...
    def __init__(self):
        ...
        style.configure("Oggi.gold.TButton", background="gold", foreground="black", font=("Arial", 8, "bold"))  # Manuale - verde chiaro
...
# 🔴 Riepilogo Mese
    ...
    def add_spesa(self):
        # ✅ Ho aggiunto un controllo per prevenire l'errore se il frame delle ricorrenze non esiste
        ...
        # ✅ Rimuovi questa riga che causa un errore se il combobox non esiste
        # self.on_categoria_changed()
    ...
    def open_compare_window(self):
        ...
        altezza = 510
        self.update_idletasks()
        self_x, self_y = self.winfo_rootx(), self.winfo_rooty()
        ...
        style.configure("Big.Treeview", font=("Arial", 10), rowheight=25)
        ...
        # === MODIFICATO: Sostituito il singolo Label con un Frame e più Labels ===
        ...
        def update_tables():
            ...
            # === NUOVO: Aggiorna le etichette dei totali con i colori corretti ===
        ...
        def do_preview_export():
            # (Il codice di questa funzione interna non necessita di modifiche)
            per_anno = compare_by_year.get()
            a1, a2 = int(left_anno.get()), int(right_anno.get())
            m1 = int(left_mese.get()) if not per_anno else 1
            m2 = int(right_mese.get()) if not per_anno else 1
    ...
    def on_stats_table_double_click(self, event):
        ...
        for d, desc, imp, tipo in sorted(spese_categoria, key=lambda x: x[0], reverse=True):
            # Crea un tag dinamico per ogni riga
            tag_name = f"row_{d.strftime('%Y%m%d%H%M%S')}_{len(tree.get_children(''))}"
            
            # Inserisce l'elemento con il tag
            tree.insert("", "end", values=(d.strftime("%d-%m-%Y"), desc, f"{imp:.2f} €", tipo), tags=(tag_name,))
            
            # Configura il tag appena creato per impostare i colori
            if tipo == "Entrata":
                tree.tag_configure(tag_name, background="#e0ffe0", foreground="green")
            else:
                tree.tag_configure(tag_name, background="#ffe0e0", foreground="red")
        ...
        # Crea un widget Text per mostrare il riepilogo con colori specifici per le cifre
        lbl = tk.Text(popup, height=1, borderwidth=0, font=("Arial", 10, "bold"), wrap="none", background=popup.cget("background"))
        lbl.pack(pady=7)
        
        # Imposta i tag per i colori
    ...
    def html_form(self):
        ...
        # --- Calcolo dei totali per il mese corrente ---
        entrate_mese = 0.0
        uscite_mese = 0.0
        oggi = datetime.date.today()
 ...
 🔹 Proiezione fine anno
  ...
  • Spesa effettiva {anno_prec}: € {stima_anno_prec:,.2f}
    ...
    def stats_mensili_html(self):
        ...
        saldo_colore = "#3c763d" if saldo >= 0 else "#c43b2e" # Verde per positivo, Rosso per negativo
    ...
    def _iconizza_finestra_startup(self):
        ...
        if splash:
            # Bind events to the splash screen to de-iconify the main window
    ...
    def _deiconify_from_splash(self, splash_window):
        ...
        self._reset_inattivita() # Optionally, restart the inactivity timer
