...
class GestioneSpese(tk.Tk):
    ...
    def genera_html_utenze(self, percorso_db, anno):
        ...
        if not os.path.exists(percorso_db):
            return "<p>❌ File database non trovato.</p>"
        try:
  ...
  • Spesa effettiva {anno_prec}: € {stima_anno_prec:,.2f}
    ...
    def __init__(self):
        ...
        btn_webserver = ttk.Button(
            ...
            text="🌐 Web port",
            width=10,
            command=self.apri_webserver_port,
            style="Webserver.TButton"
        )
        btn_webserver.pack(side="left", padx=(0, 5))
    ...
    def aggiorna_combobox_categorie(self):
        """Ordina le categorie alfabeticamente, tenendo 'Generica' in cima se presente."""
        if "Generica" in self.categorie:
            # Escludi 'Generica' dalla lista da ordinare
            altre = [c for c in self.categorie if c != "Generica"]
            # Crea nuova lista con 'Generica' in testa
            ordinata = ["Generica"] + sorted(altre, key=str.lower)
        else:
            # Nessuna 'Generica', ordina tutto normalmente
            ordinata = sorted(self.categorie, key=str.lower)
    ...
    def apri_webserver_port(self):
        ...
        if os.path.exists(PORTA_DB):
            try:
                ...
                pass
        vcmd = (finestra.register(lambda val: val.isdigit()), "%P")
