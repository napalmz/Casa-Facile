...
# ⏱️ 2 minuti in millisecondi
TIMEOUT_INATTIVITA_MS = 120000
...
class CasaFacileWebHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        elif path.startswith("/menu_ricerca"):
            html = self.server.app.pagina_menu_ricerca()
    ...
    def do_POST(self):
        ...
        if path == "/":
            ...
            voce = {
                "date": params.get("data", ["01-01-2024"])[0],  # 👈 la data è ora parte della voce
        ...
        if path == "/salva_modifica":
            ...
            self.server.app.refresh_gui()  # 👉 aggiornamento interfaccia
            self.send_response(303)
        ...
        if path == "/cancella":
            ...
            self.send_response(303)
            self.send_header("Location", "/lista")  # reindirizza alla lista aggiornata
            self.end_headers()
            return
            self.server.app.aggiungi_voce_web(voce)  # ✅ versione blindata
...
class GestioneSpese(tk.Tk):
    ...
    def pagina_risultati_avanzati(self, params):
        ...
        for data in sorted(self.spese.keys(), reverse=True):
            ...
            for voce in voci:
                ...
                cat = voce[0].strip().lower()
                descr = voce[1].strip().lower()
                importo = voce[2]
                tipo = voce[3].strip().lower()

                if categoria and cat != categoria: continue
                if not (min_importo <= importo <= max_importo): continue
                if query and not (
                    query in descr or query in tipo or query in cat or query in str(importo)
                ):
                    continue
        ...
        # 💰 Totali calcolati dalle voci filtrate
        entrate_totali = sum(voce["importo"] for voce in risultati if voce["tipo"].strip().lower() == "entrata")
        uscite_totali = sum(voce["importo"] for voce in risultati if voce["tipo"].strip().lower() != "entrata")
        ...
        for voce in risultati:
            schede_html += f"""
            <div class="spesa">
                <div><strong>Data:</strong> {voce['data']}</div>
                <div><strong>Categoria:</strong> {voce['categoria']}</div>
                <div><strong>Descrizione:</strong> {voce['descrizione']}</div>
                <div><strong>Importo:</strong> €{voce['importo']:.2f}</div>
                <div><strong>Tipo:</strong> {voce['tipo']}</div>
            </div>
            """
    ...
    def pagina_risultati_avanzati(self, params):
        ...
        for data in sorted(self.spese.keys(), reverse=True):
            ...
            for voce in self.spese[data]:
                if len(voce) < 4: continue
                cat, descrizione, importo, tipo = voce
                if categoria and cat.strip().lower() != categoria: continue
                if not (min_importo <= importo <= max_importo): continue
                if query and not (query in descrizione.lower() or query in tipo.lower() or query in cat.lower() or query in str(importo)):
                    continue
                risultati_categorizzati[cat].append((data.strftime("%d-%m-%Y"), descrizione, importo, tipo))
        ...
        schede_html = ""
        for idx, (cat, voci) in enumerate(risultati_categorizzati.items()):
            totale_cat = sum(imp for _, _, imp, _ in voci)
            voce_html = ''.join(f"<li>{data} • {descrizione} — €{importo:.2f}</li>" for data, descrizione, importo, _ in voci)
            schede_html += f"""
            <div class="categoria-blocco">
                <button class="toggle-btn" onclick="toggleCategoria(this)">
                    ➤ {cat} — Totale: €{totale_cat:.2f}
                </button>
        ...
        <head>
            ...
            function toggleCategoria(btn) {{
                ...
                btn.textContent = (isVisible ? "➤" : "▼") + btn.textContent.slice(1);
            }}    
            </script>
        </head>
        <body>
            <h2>📊 Totali della ricerca</h2>
            <ul class="totali">
            ...
            <a href="/menu_ricerca" class="back">🔙 Torna al Menu Ricerca</a>
    ...
    def html_form(self):
        ...
        <body>
            <header>🏠 Casa Facile Web</header>
            <main>
                <form method="post" id="spesaForm">
                    ...
                    <input name="importo" type="number" step="0.01" placeholder="Es: 12.50">
                ...
                <form method="get" action="/lista">
                    <input type="submit" value="📈 Elenca/Modifica Spese del Mese">
                </form>
                <form method="get" action="/stats">
                    <input type="submit" value="📊 Mostra Statistiche Mese">
                </form>
                <form method="get" action="/menu_ricerca">
                    <input type="submit" value="💰 >🔎 Ricerca avanzata">
                </form>
                <form method="get" action="/logoff">
    ...
    def pagina_menu_ricerca(self):
        mesi = [f"{m:02d}" for m in range(1, 13)]
        anni = sorted(set(str(data.year) for data in self.spese.keys()))
        ...
        <body>
            <header>🔎 Ricerca avanzata</header>
            <main>
    ...
    def html_lista_spese_mensili(self):
        ...
        <head>
            <meta charset="utf-8">
            <title>📊 Casa Facile — Spese di {mese_it} {oggi.year}</title>
            <meta name="viewport" content="width=device-width, initial-scale=1">
            <style>
                body {{
                    font-family: 'Segoe UI', sans-serif;
        ...
        <body>
            <div class="container">
                ...
                <form method="post" action="/salva_modifica">
                    ...
                    <label for="categoria">Categoria</label>
                    <input name="categoria" value="{categoria}" required>
    ...
    def __init__(self):
        ...
        #iconizza
        #self.deiconify()   # 👈 Mostra normalmente
        #Timeout Iconizza
        ...
        style.configure("Report.TButton", background="yellow", foreground="black")  # Report - verde chiaro
        ...
        btn_webserver = ttk.Button(
            barra_azione,
            text="🌐 Apri WebServer",
            width=25,
            command=self.apri_webserver,
            style="Webserver.TButton"
        )
        btn_webserver.pack(side="left", padx=(0, 5))

        btn_manuale = ttk.Button(
            barra_azione,
            text="🌐 Apri Manuale",
            width=25,
            command=self.scarica_manuale,
            style="Manuale.TButton"
        )
        ...
        totali_row.pack(fill=tk.X, padx=6, pady=(2,0))

        self.totali_label = ttk.Label(totali_row, text="", font=("Arial", 11, "bold"))
        self.totali_label.pack(side=tk.LEFT)
        
        

        self.considera_ricorrenze_var = tk.BooleanVar(value=True) #Considera sempre le spese ricorrenti nel bilancio
        ...
        self.stats_table.bind("<Double-1>", self.on_stats_table_double_click) # Doppio click mese anno totali
        for col in ("A", "B", "C", "D", "E", "F"):
            self.stats_table.heading(col, text=col, command=lambda _col=col: self.treeview_sort_column(self.stats_table, _col, False))
        ...
        style.configure("AggiungiSpesa.TButton", background="#32CD32", foreground="black")
        style.configure("SalvaModifica.TButton", background="#32CD32", foreground="black")
        style.configure("CancellaVoce.TButton", background="red", foreground="black")
        ...
        style.configure("SuggerisciCategoria.TButton", background="#FFA500", foreground="black")  # Suggerisci Categoria - arancione
        ...
        ttk.Button(aggiungi_cat_frame, text="🔍 Aggiungi Categoria", command=self.add_categoria, style="AggiungiCategoria.TButton").grid(row=0, column=2, padx=8, pady=6)
        
        ttk.Button(aggiungi_cat_frame, text="✨ Suggerite", command=self.apri_categorie_suggerite, style="SuggerisciCategoria.TButton").grid(row=0, column=3, padx=8, pady=6)
    ...
    def aggiorna_titolo_finestra(self):
        ...
        current_folder = os.path.basename(os.getcwd())
        self.title(f"💰 Casa Facile Pro v {VERSION}  — Email: helpcasafacilepro@gmail.com —  {now}  —  Utente:► {current_folder}")
    ...
    def mostra_spese_simili(self):
        ...
        try:
            ...
            self.show_custom_warning("Importo non valido", "Inserisci un importo numerico.")
            return
    ...
    def show_export_preview(self, content):
        ...
        btn_salva = tk.Button(btn_frame, text="💾 Salva", command=save_file, width=15, bg="forestgreen", fg="white")
    ...
    def open_compare_window(self):
        ...
        anni = sorted(set(range(anno_corrente-10, anno_corrente+11)).union(anni_spese))
        ...
        tk.Button(btnframe, text="📄 Preview/Esporta", command=do_preview_export, width=18, bg="orange", fg="white").pack(side=tk.LEFT, padx=8)
    ...
    def show_info_app(self):
        ...
        text.insert("end", "Casa FacilePro\n", "titolo")
        ...
        text.insert("end", "  sudo apt install tkcalendar python3-psutil\n")
        text.insert("end", "  Oppure:\n")
        text.insert("end", "  pip install tkcalendar psutil\n")

        text.insert("end", "\nSu Windows:\n")
        text.insert("end", "  Apri il Prompt dei comandi e digita:\n")
        text.insert("end", "  py -m pip install tkcalendar psutil win32print win32api win32con\n")
        text.insert("end", "  Assicurati di installare Python, psutil, tkcalendar, win32print, win32api e win32con prima di avviare il programma.\n")
        ...
        btn_aggiorna = tk.Button(info_win, text="🔄 Aggiorna Software", command=lambda: self.aggiorna(GITHUB_FILE_URL, NOME_FILE), bg="orange", fg="white")
    ...
    def utenze(self):
        ...
        for idx, utenza in enumerate(utenze):
            ...
            tk.Button(btns, text="💾 Salva", width=9, command=salva_dati, bg="forestgreen", fg="white").pack(side=tk.LEFT, padx=2)
    ...
    def cerca_operazioni(self):
        ...
        tk.Button(frame_inferiore, text="❌ Chiudi", command=chiudi, bg="gold", fg="black").pack(side="right")
        ...
        def esegui_ricerca(event=None):
            ...
            for data_key in sorted(self.spese.keys(), reverse=True):
                ...
                # FILTRO spese future se la checkbox è tolta
                if not mostra_futuro and data > oggi:
                    continue
        ...
        tk.Button(frame_inferiore, text="🔍 Cerca", command=esegui_ricerca, bg="forestgreen", fg="white").pack(side="left", padx=6)
    
        def esporta_risultato():
            contenuto = area_testo.get(1.0, tk.END).strip()
            if not contenuto:
                self.show_custom_warning("Esporta", "⚠️ Nessun risultato da salvare.")
                return
        ...
        tk.Button(frame_inferiore, text="📄 Esporta", command=esporta_risultato, bg="orange", fg="white").pack(side="right")
    ...
    def gruppo_categorie(self):
        ...
        ttk.Combobox(top_bar, values=mesi, textvariable=mese_var, state="disabled", width=12).pack(side="left")
        ...
        ttk.Combobox(top_bar, values=[str(a) for a in anni], textvariable=anno_var, state="disabled", width=8).pack(side="left")
    ...
    def on_stats_table_double_click(self, event):
        ...
        label = tk.Label(popup, text=f"Spese categoria '{categoria}' per {testo_periodo}", font=("Arial", 11, "bold"))
        label.pack(pady=8)
        ...
        for col, w in zip(columns, (90, 230, 100, 80)):
            tree.heading(col, text=col)
            tree.column(col, width=w, anchor="center" if col != "Descrizione" else "w")
    ...
    def mostra_report_popup(self, testo):
        ...
        tk.Button(frame_bottoni, text="📄 Esporta", bg="#32CD32", fg="white", width=12, command=save_file).pack(side="left")
        tk.Button(frame_bottoni, text="Chiudi", bg="yellow", width=12, command=preview.destroy).pack(side="right")
        preview.bind("<Escape>", lambda e: preview.destroy())
...
if __name__ == "__main__":
    ...
    print("""
    📘 Casa Facile — Guida agli argomenti da riga di comando
    ...
    Esempi:
    CasaFacile.pyw auto 1234      # imposta password “1234”
    Casa Facile.pyw auto ""       # password vuota
    Casa Facile.pyw auto          # password automatica (definita da utente)
    Casa Facile.pyw noweb         # GUI senza web server
    Casa Facile.pyw auto "" noweb # password vuota + niente web
