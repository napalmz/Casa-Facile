 ...
 🔹 Proiezione fine anno
  ...
  • Spesa effettiva {anno_prec}: € {stima_anno_prec:,.2f}
    ...
    def __init__(self):
        ...
        menu_funzioni.add_command(label="🖨️ Stampa", accelerator="Ctrl+P", command=self.anteprima_e_stampa_txt)
        ...
        menu_funzioni.add_command(label="🕰️ Time Machine", accelerator="Ctrl+T", command=self.time_machine)
        ...
        menu_estrazioni.add_command(label="📅 Estrai Giorno", accelerator="Ctrl+1", command=self.export_giorno_forzato)
        self.bind_all("<Control-1>", lambda e: self.export_giorno_forzato())
        menu_estrazioni.add_command(label="📅 Estrai Mese", accelerator="Ctrl+2", command=self.export_month_detail)
        self.bind_all("<Control-2>", lambda e: self.export_month_detail())
        menu_estrazioni.add_command(label="📊 Estrai Anno", accelerator="Ctrl+3", command=self.export_anno_dettagliato)
        self.bind_all("<Control-3>", lambda e: self.export_anno_dettagliato())
        ...
        menu_setup.add_command(
            ...
            command=lambda: (
                ...
                #messagebox.showinfo("Impostazione salvata", "Modalità Semplice attivata.\n Riavviare interfaccia...")
                self.save_db(),
                self.destroy(),
        ...
        menu_setup.add_command(
            ...
            command=lambda: (
                salva_modalita("avanzata"),
                 #messagebox.showinfo("Impostazione salvata", "Modalità Avanzata attivata.\n Riavviare interfaccia..."),
                 self.save_db(),
                 self.destroy(),
                 self.__class__().mainloop()
            )
        ...
        menu_db.add_command(label="⚙️ Reset DB", command=self.show_reset_dialog)
        # ⚙️ Menu Info
        ...
        # Stili pulsanti
        style.configure("Giorno.TButton", background="#00FFFF", foreground="black")
        style.configure("Mese.TButton", background="#00FFFF", foreground="black")
        style.configure("Anno.TButton", background="#00FFFF", foreground="black")
        style.configure("Totali.TButton", background="#FF6666", foreground="black")
        style.configure("Categoria.TButton", background="#90EE90", foreground="black")
        style.configure("Esporta.TButton", background="#ADD8E6", foreground="black")
        ...
        self.stats_table = ttk.Treeview(table_container, columns=("A", "B", "C", "D", "E", "F"), show="headings")
    ...
    def aggiorna_titolo_finestra(self):
        current_folder = os.path.basename(os.getcwd())
        self.title(f"💰 Casa Facile Pro v {VERSION}  — Email: helpcasafacilepro@gmail.com —  Utente:► {current_folder}")
    ...
    def cerca_operazioni(self):
        ...
        def esporta_risultato():
            ...
            if not contenuto:
                ...
                return
            default_dir = EXPORT_FILES
            nome_file = f"Risultati_Ricerca_{datetime.date.today():%Y_%m_%d}.txt"
            file = filedialog.asksaveasfilename(
                defaultextension=".txt",
                filetypes=[("File di testo", "*.txt")],
                initialdir=default_dir,
    ...
    def check_aggiornamento_con_api(self):
        ...
        try:
            ...
            if remote_time.date() > local_time.date():
                ...
                # 📍 Centramento e visualizzazione coerente
                win.withdraw()
                win.update_idletasks()
                min_w, min_h = 460, 200
