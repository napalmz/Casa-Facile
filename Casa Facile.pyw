...
class GestioneSpese(tk.Tk):
    ...
    def __init__(self):
        ...
        style.configure("Report.TButton", background="yellow", foreground="black")  # Stampa - verde chiaro
    ...
    def treeview_sort_column(self, tv, col, reverse):
        ...
        try:
            # Prova a convertire in float (numeri con virgola o simbolo €)
            items.sort(key=lambda t: float(str(t[0]).replace(",", ".").replace("€", "")), reverse=reverse)
        except Exception:
            # Altrimenti ordina come testo
        ...
        if mode == "giorno":
            ...
            for d, sp in self.spese.items():
                ...
                if mode == "mese":
                    ...
                    if d.year != ref.year:
                        continue
                # Nessun filtro per "totali" (tutto l'anno)

                for entry in sp:
                    data_voce = d

                    # Filtro ricorrenze future secondo regole richieste (se la checkbox è SPENTA)
                    if not self.considera_ricorrenze_var.get():
                        if mode == "totali":
                            ...
                            if ref.year == oggi.year:
                                if data_voce > oggi:
                                    continue
                            # Se ref.year < oggi.year: mostra tutto
                        elif mode == "mese":
                            if ref.year == oggi.year and ref.month == oggi.month:
                                if data_voce > oggi:
                                    continue
                            # Se il mese è passato: mostra tutto
            ...
            # Inserisci i totali per categoria nella tabella e calcola i totali generali
    ...
    def update_totalizzatore_anno_corrente(self):
        ...
        for d, sp in self.spese.items():
            if d.year == anno:
                for entry in sp:
                    # Filtro ricorrenze future solo se c'è la variabile e la checkbox è disattivata
    ...
    def update_totalizzatore_mese_corrente(self):
        ...
        for d, sp in self.spese.items():
            if d.year == year and d.month == month:
                for entry in sp:
                    # Filtro ricorrenze future come nella funzione annuale
                    if hasattr(self, "considera_ricorrenze_var") and not self.considera_ricorrenze_var.get() and len(entry) == 5:
                        if d > now:
                            continue
                    tipo = entry[3]
    ...
    def open_compare_window(self):
        ...
        # Prepara lista anni/mesi disponibili
        anni_spese = set()
        ...
        # Aggiorna su ogni cambio di variabile
    ...
    def show_info_app(self):
        ...
        # Posizionamento finestra centrato
        info_win.withdraw()
    ...
    def utenze(self):
        ...
        def apri_modale(utenza):
            ...
            # 👇 Rende le variabili accessibili da salva()
            modal.e_prec = e_prec
            modal.e_att = e_att
    ...
    def apri_finestra_importa(self):
        ...
        win.resizable(False, False)
        win.grab_set()
        win.configure(bg="#F9F9D1")  # Facoltativo: sfondo giallo chiaro
        ...
        style = ttk.Style()
        style.configure("ICC.TButton", foreground="black", background="#FFA500")  # Arancione classico
        style.map("ICC.TButton", background=[("active", "#FF8C00")])  # Arancione scuro su hover
    ...
    def importa_spese_csv_unicredit(self):
        ...
        # 🔧 Pannello per aggiungere nuove categorie al volo
    ...
    📅 Mese corrente: {mese_corr:02}/{anno_corr}
      ...
      ▸ Uscite: € {tot_mese_corr:,.2f}
      ▸ Stesso mese {anno_prec}: € {tot_mese_prec:,.2f}

    ──────────────────────────────────────────────────────────────────────────────────── 
    📆 Da 01/01 a oggi
    
      ▸ Totale uscite {anno_corr}: € {tot_anno_corr:,.2f}
      ▸ Stesso periodo {anno_prec}: € {tot_anno_prec:,.2f}
    ──────────────────────────────────────────────────────────────────────────────────── 
    
    📈 Spesa a confronto
    
      ▸ Spesa effettiva nel {anno_prec}: € {stima_2024:,.2f}
      ▸ Proiezione per il {anno_corr}: € {stima_2025:,.2f}   (basata su {perc_corr:.1%} dell’anno trascorso)
    ──────────────────────────────────────────────────────────────────────────────────── 
    """
    ...
    def mostra_report_popup(self, testo):
        ...
        # Centra la finestra rispetto alla principale
        self.update_idletasks()
        parent_x = self.winfo_rootx()
        parent_y = self.winfo_rooty()
        parent_w = self.winfo_width()
