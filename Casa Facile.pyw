...
class GestioneSpese(tk.Tk):
    ...
    def __init__(self):
        ...
        ttk.Button(topbar, text="📤 Importa DB", command=self.import_db, style="Importa.TButton").pack(side=tk.RIGHT, padx=6)
        ttk.Button(topbar, text="📤 Esporta DB", command=self.export_db, style="Esporta.TButton").pack(side=tk.RIGHT, padx=6)
        ttk.Button(topbar, text="🔄 Reset Database", command=self.show_reset_dialog, style="Reset.TButton").pack(side=tk.RIGHT, padx=6)

        ttk.Button(topbar, text="ℹ️ Info", command=self.show_info_app, style="Info.TButton").pack(side=tk.LEFT, padx=6)
        ttk.Button(topbar, text="💰 Saldo", command=self.open_saldo_conto, style="Saldo.TButton").pack(side=tk.LEFT, padx=6)
        ttk.Button(topbar, text="🔍 Confronta", command=self.open_compare_window, style="Confronta.TButton").pack(side=tk.LEFT, padx=6)
        ttk.Button(topbar, text="👤 Utenze", command=self.utenze, style="Utenze.TButton").pack(side=tk.LEFT, padx=6)
        ttk.Button(topbar, text="📅 Rubrica", command=self.rubrica_app, style="Rubrica.TButton").pack(side=tk.LEFT, padx=6)
        ttk.Button(topbar, text="📥 Stampa", command=self.anteprima_e_stampa_txt, style="Stampa.TButton").pack(side=tk.LEFT, padx=6)
        ttk.Button(topbar, text="📥 Cerca", command=self.cerca_operazioni, style="Cerca.TButton").pack(side=tk.LEFT, padx=6)
        ttk.Button(topbar, text="📥 T.M.", command=self.time_machine, style="TM.TButton").pack(side=tk.LEFT, padx=4)
    ...
    def aggiorna_titolo_finestra(self):
        ...
        self.title(f"Casa Facile Pro v {VERSION}  —   {now}")
    ...
    def show_reset_dialog(self):
        ...
        label = tk.Label(dialog, text=(
            ...
            "'Solo Rubrica' per cancellare tutta la rubrica\n"
        ...
        def do_cancel():
            dialog.destroy()
        tk.Button(btns, text="✅ Sì", command=do_yes, width=6, bg="#ff6666", fg="black").grid(row=0, column=0, padx=5)
        tk.Button(btns, text="❌ No", command=do_no, width=6, bg="#ffff66", fg="black").grid(row=0, column=1, padx=5)
        tk.Button(btns, text="📅 Solo letture", command=do_letture, width=13, bg="#ffcc66", fg="black").grid(row=0, column=2, padx=5)
        tk.Button(btns, text="📅 Solo rubrica", command=do_rubrica, width=13, bg="#cc99ff", fg="white").grid(row=0, column=3, padx=5)
        tk.Button(btns, text="❌ Annulla", command=do_cancel, width=10, bg="#99ff99", fg="black").grid(row=0, column=4, padx=5)
    ...
    def cerca_operazioni(self):
        ...
        # Frame top: descrizione + campo input + pulsanti azione
        frame_superiore = tk.Frame(finestra)
        frame_superiore.pack(fill="x", pady=10, padx=10)

        label_descrizione = tk.Label(frame_superiore, text="🔎 Ricerca globale: qualsiasi campo, qualsiasi voce: '⏎'")
        label_descrizione.pack(side="left")

        campo_input = tk.Entry(frame_superiore, width=20)
        campo_input.pack(side="left", padx=(15, 0))
        campo_input.focus_set()

        #tasto_return = tk.Label(frame_superiore, text="⏎", fg="gray")
        #tasto_return.pack(side="left", padx=5)
        ...
        width=2,  # Riduce la larghezza del bottone
        padx=2,   # Margine interno orizzontale più stretto
        pady=0,
        font=("Arial", 10)
        )
        bottone_reset.pack(side="left", padx=(10, 0))
        ...
        # Frame inferiore: bottoni Chiudi e Cerca
        ...
        def esegui_ricerca(event=None):
            ...
            if not parola:
                area_testo.insert(tk.END, "⚠️ Inserisci una parola da cercare.")
                return
            ...
            for data in sorted(self.spese.keys(), reverse=True):
                ...
                for voce in voci:
                    if len(voce) >= 2:
                        ...
                        importo = voce[2] if len(voce) > 2 else ""
                        tipo = voce[3] if len(voce) > 3 else ""
                        if parola in categoria or parola in descrizione:
                            riga = "\n"
                            riga += f"📅 Data operazione : {data.strftime('%d/%m/%Y')}\n"
                            riga += f"{'':2}Categoria         : {voce[0]}\n"
                            riga += f"{'':2}Descrizione       : {voce[1]}\n"
                            riga += f"{'':2}Tipo              : {tipo:<10}    Importo: {float(importo):>10,.2f} €\n"
                            riga += f"{'═'*80}\n"
            ...
            if risultati:
                for riga in risultati:
                     area_testo.insert(tk.END, riga)
            else:
                area_testo.insert(tk.END, f"🔍 Nessuna corrispondenza per: '{parola}'")
    ...
    def time_machine(self):
        ...
        ttk.Label(top_bar, text="Anno:", font=("Segoe UI", 10)).pack(side="left", padx=(0, 5))
        ...
        ttk.Label(sinistra, text="🎯 Selezione manuale", font=("Segoe UI", 10, "bold")).pack(anchor="w", pady=(0, 6))
        destra_label = ttk.Label(destra, text="📊 Top 10 categorie per spesa", font=("Segoe UI", 10, "bold"))
        ...
        ttk.Label(main, text="💡 Risultato della simulazione:", font=("Segoe UI", 10, "bold")).pack(anchor="w", padx=5)
