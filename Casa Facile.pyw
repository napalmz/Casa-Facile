...
class GestioneSpese(tk.Tk):
    ...
    def __init__(self):
        ...
        self.suggerimenti_attivi = False # ⛔ Disattiva suggerimento categoria=spesa
    ...
    def aggiorna_categoria_automatica(self, event=None):
        if not self.suggerimenti_attivi:
            self.label_smartcat.config(text="🛠️ SmartCat disattiva", foreground="green")
            self.aggiorna_bottone_spese_simili(visibile=False)
            return  # ⛔ Suggerimenti disattivati
        valore = self.imp_entry.get().replace(",", ".").strip()
        if not valore:
            self.categoria_bloccata = False  # Il campo è vuoto → suggerimento pronto alla ripartenza
            self.aggiorna_bottone_spese_simili(visibile=False)
            return
        if not valore:
            self.categoria_bloccata = False  # ✅ Sblocca se importo svuotato
            return
        ...
        # Cerca tra tutte le spese caricate
        ...
        if categoria_migliore and not self.categoria_bloccata:
            ...
            self.on_categoria_changed(manuale=False)  # 🔁 aggiorna il tipo e lo stile
            self.label_smartcat.config(text="💡 SmartCat attiva", foreground="red")
            self.btn_spese_simili.pack(side="left", padx=(6, 0))  # 👈 mostra pulsante
            # Evidenziazione rossa temporanea sul combobox
            style = ttk.Style()
            style.configure("Highlight.TCombobox", foreground="red")  # rosso
    ...
    def gestione_login(self):
        ...
        def mostra_finestra_login():
            ...
            if os.path.exists(PW_FILE):
                  ...
                  tk.Label(login, text="🆕 PRIMO ACCESSO \n\n🔐 Crea una nuova password per proteggere i tuoi dati.\n Premi Invio per confermare.", font=("Arial", 10)).pack(pady=(15, 5))
