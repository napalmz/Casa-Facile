...
class GestioneSpese(tk.Tk):
    ...
    def __init__(self):
        ...
        style.configure("ICC.TButton", background="red", foreground="black")
        style.configure("CCV.TButton", background="red", foreground="black")
        ...
        self.btn_importa_carta = ttk.Button(pannello_bottoni, text="💸 ICC", command=self.importa_spese_csv, width=6, style="ICC.TButton")
        self.btn_importa_carta.pack(side="left", padx=2)

        self.btn_importa_cc = ttk.Button(pannello_bottoni, text="💸 CCV", command=self.importa_spese_cc_csv, width=6, style="CCV.TButton")
        self.btn_importa_cc.pack(side="left", padx=2)
        ...
        def salva():
            ...
            for mov, var, combo in righe:
                if var.get():
                    ...
                    # Verifica duplicati su: categoria + importo + tipo nella stessa data
                    gia_presente = any(
                        cat == v[0] and
                        abs(imp - v[2]) < 0.01 and
                        tipo == v[3]
                        for v in self.spese.get(giorno, [])
                    )
                    if gia_presente:
                        duplicati += 1
                        continue
