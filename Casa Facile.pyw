...
class GestioneSpese(tk.Tk):
    ...
    def mostra_ricorrenza_popup(self):
        ...
        def convalida_ric_n(valore):
            if valore == "":
                return True
            return len(valore) <= 4 and re.fullmatch(r"^\d{0,4}$", valore) is not None
        vcmd_ric = ripeti_frame.register(convalida_ric_n)
...
# Pulsante "Calcola" con la sua larghezza normale
    ...
    def time_machine(self):
        ...
        def aggiorna_interfaccia(*_):
            ...
            tutte_categorie_da_spese = contatori.keys()
            tutte_categorie_principali = self.categorie_tipi.keys()
